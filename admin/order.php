...
<body>
    <div class="sidebar">
        ...
        <a class="active" href="./order.php"><i class="fas fa-luggage-cart mx-1"></i>Quản lý đơn hàng</a>
    ...
    <div class="content container-fluid">
        <div class="row mt-2">
            ...
            <div class="col-sm-3">
                <button class="btn btn-primary" type="button" onclick="searchOrder()"><i class="fa fa-search"></i> Tìm kiếm</button>
            ...
            <div class="col-md-10 mx-3">
                <table class="table table-striped">
                    ...
                    <tbody id="tblCustomer">
                        <?php
                            include '../config.php';
                            $sql = $sql = "select orderr.*, product.image from orderr, product where orderr.productID = product.ID";
                            ...
                            if(mysqli_num_rows($result) > 0){ 
                                    ...
                                    while($results = mysqli_fetch_array($result)){
                                        ...
                                        echo '<td class="text-center">'.$results['totalPrice'].'</td>';
                                        if($results['status'] == 1){
                                            echo '<td class="text-center">Đã duyệt</td>';
                                        }
                                        else echo '<td class="text-center">Chưa duyệt</td>';
                                        echo '<td class="text-center">';
                                        if($results['status'] == 0){
                                            echo '<i class="far fa-check-circle mx-1" style="color: #8ee068;" onclick="confirmOrder(';
                                            echo $results['id'];
                                            echo ')"></i>';
                                        }
                                        echo '<i class="fas fa-edit mx-1" style="color: #FDA24F;" onclick="editOrder(';
                                        echo $results['id'];
                                        echo ')"></i>';
                                        echo '<i class="fas fa-minus-circle mx-1" style="color: #E14B67"; onclick="confirmDeleteOrder(';
                                        echo $results['id'];
                                        echo ')"></i>';
                                        echo "</td>";
                                        echo "</tr>";
